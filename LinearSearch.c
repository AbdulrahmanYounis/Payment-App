...
uint8_t linearSearch(struct account_balance *dataBase, uint8_t size, uint8_t *target) {
        for (int i = 0; i < size; i++) {
            // check whether the two strings are equal or not
            if (strcmp((char *) dataBase[i].PAN, (char *) target) == 0)
...
// function used to print the array-> used to show the update in database after each transaction
    void printArray(struct account_balance *dataBase, uint32_t size) {
        printf("\n");
        for (int i = 0; i < size; i++)
            printf("%s\t%0.2f\n", dataBase[i].PAN, dataBase[i].balance);
