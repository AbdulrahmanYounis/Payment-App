...
uint8_t server(struct account_balance *dataBase,struct card_data *card, struct terminal_data *t) {
    // using linear search algorithm to search for the primary account number user entered
    uint8_t result = linearSearch(dataBase, 10, card->PAN);
    ...
    if(dataBase[result].balance < t->TA){
        printf("The Transaction is DECLINED\n");
